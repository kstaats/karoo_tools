...
while True:
    try:
        skew_sel = input('\t Skew the data across the origin? y/n (default y): ')
...
while True:
    try:
        class_type = input('\t Select (i)nfinite or (f)inite wing bins (default i): ')
...
while True:
    try:
        class_labels = input('\t Enter the number of class labels / solutions (default 4): ')
...
if skew_sel == 'y':
    ...
    print('\n\t solutions =', list(range(class_labels)))
