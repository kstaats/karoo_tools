...
while True:
    try:
        clean = input('\t Clean your data by (c)olumn or by (r)ow (default r): ')
...
value = input('\t Enter the value you seek in the data (default nan): '); value = str(value) or 'nan'

while True:
    try:
        d_type = input('\t Are you seeking a (s)tring or a (f)loat? (default s): ')
...
if clean == 'r': # clean rows
    ...
    if np.sum(o) == 0: print('\t No columns contain the value:\033[36m', value, '\033[0;0m'); sys.exit() # changed .any to .sum 20170929
        ...
        menu = list(range(0,101))
        while True:
            try:
                threshold = input('\t What is the minimum percental for removal of a column? (default 0): ')
